...
type CheckoutTab = "gift-codes" | "status";
...
const TABS: ReadonlyArray<{ id: CheckoutTab; label: string }> = [
...
export default function CheckoutPage() {
  const [tab, setTab] = useState<CheckoutTab>("gift-codes");
    ...
    <div className="p-8 lg:p-12">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm text-neutral-500">
          Issue gift codes for free convenience-fee checkouts, or review
          refund-candidate paid sessions.
        </p>
      </div>

      <div
        role="tablist"
        aria-label="Checkout views"
        className="mt-8 inline-flex rounded-xl bg-neutral-100 p-1"
      >
        {TABS.map((item) => {
      ...
      <div
        role="tabpanel"
        aria-labelledby={`checkout-tab-${tab}`}
        className="mt-8"
      >
        {tab === "gift-codes" ? <GiftCodesSection /> : <StatusSection />}
      </div>
    </div>
  );
}
